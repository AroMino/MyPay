...
public class Bank extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String r = "SELECT SUM(value) as value from transactions where login='"+login+"' and field='bank'";
		...
		if(transaction.getType() != null) {
			...
			if(transaction.getType().equals("withdrawal") || transaction.getType().equals("transfer_to") || transaction.getType().equals("payment")) value *=-1;
			transaction.setValue(value);
			transaction.setField("bank");
			...
			try {
				...
				response.sendRedirect("Bank?login="+login);
			}
		}
		...
		try {
            ...
            r = "SELECT * from transactions where login='"+login+"' and field='bank'";
